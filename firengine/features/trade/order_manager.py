...
class OrderManager:
    def __init__(self, ex: ccxt.Exchange):
        ...
        self._open_orders_per_symbol: defaultdict[str, dict[str, Order]] = defaultdict(dict)
        self._closed_orders_per_symbol: defaultdict[str, dict[str, Order]] = defaultdict(dict)
    ...
    def get_open_order(self, symbol: str, order_id: str) -> Order | None:
        return self._open_orders_per_symbol[symbol].get(order_id)
    ...
    def get_closed_orders_per_symbol(self, symbol: str) -> dict[str, Order]:
        return self._closed_orders_per_symbol[symbol]
    ...
    async def on_trade_updated(self, trade: PrivateTrade):
        self._trades_per_order[trade.order][trade.id] = trade

    async def add_symbol(self, symbol: str):
        if symbol not in self._symbols:
            self._symbols.add(symbol)
            self._open_orders_per_symbol[symbol] |= await self._fetch_open_orders(self._exchange, symbol)
            self._closed_orders_per_symbol[symbol] |= await self._fetch_closed_orders(self._exchange, symbol)
    ...
    async def modify_order_info(self, target: OrderInfo):
        await self.edit_order(
            target.id, target.symbol, target.type, target.side, target.amount, target.price
        )

    async def modify_order(self, target: Order):
        await self.edit_order(target.id, target.symbol, target.type, target.side, target.amount, target.price)
